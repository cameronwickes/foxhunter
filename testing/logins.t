...
class TestLogins(unittest.TestCase):
    ...
    def testPasswordAuthentication(self):
        ...
        cmd = fox.getScript() + ["-p", self.profile, "-oC", self.outputDir, "-oJ", self.outputDir, "-oX", self.outputDir]
        _ = fox.run(cmd, workdir=fox.getProfiles(), stdin="1\n{}\n1\n{}\n".format("PasswordAuthFailure", self.password)) 
        
        for fileType in ["csv", "json", "xml"]:
            with open(os.path.join(self.outputDir, "logins.{}".format(fileType))) as actualOutput:
                with open(
    ...
    def testBruteForce(self):
        ...
        cmd = fox.getScript() + ["-p", self.profile, "-oC", self.outputDir, "-oJ", self.outputDir, "-oX", self.outputDir]
        _ = fox.run(cmd, workdir=fox.getProfiles(), stdin="2\n{}\n2\n{}".format(os.path.join(fox.getProfiles(), "wordlistWithoutPassword"), os.path.join(fox.getProfiles(), "wordlistWithPassword"))) 
